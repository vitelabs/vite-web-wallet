    ...
    <div class="bridge-content">
      ...
      <div class="__row" v-if="!requireBSCConnect">
        <div class="__row-tips">
          ...
          <div>
            The maximum bridge amount is {{ richNetworkPair.from.max }}
            {{ curToken.label }}
          </div>
          <div>
            The minimum bridge amount is {{ richNetworkPair.to.min }}
            {{ curToken.label }}
          </div>
        </div>
      </div>
  ...
  computed: {
    ...
    tokenInfos() {
      ...
      return (this.tokens || []).map((t) => {
        const tokenId = t.channels[0].find(
          (c) => c.network === "VITE"
        )?.tokenId;
        return {
          ...t,
    ...
    async onNextClick() {
      ...
      async function checkApprove() {
        if (curNet === "BSC") {
          ...
          const approved = bnUtils.compared(allowance.toString(), ammountMin);
          return approved >= 0;
        }
        return true;
      }
