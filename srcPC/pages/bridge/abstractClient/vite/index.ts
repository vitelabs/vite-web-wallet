...
export class ChannelVite {
  ...
  async scanEvents(fromHeight: string, eventName: string) {
      ...
      const eventAbi = this.viteChannelAbi.find((item: { name: string; type: string }) =>
      ...
      return {
          ...
          events: events.map((input: any) => {
              const event: any = this.decodeEvent(input.vmlog,
                  this.viteChannelAbi,
                  eventName);
              return {
                  event: event,
  ...
  decodeEvent(log: any,
      channelAbi: Array<{ name: string; type: string }>,
      name: string) {
      const result = abi.decodeLog(channelAbi,
          Buffer.from(log.data ? log.data : '', 'base64').toString('hex'),
          log.topics.slice(1, log.topics.length),
          name);
  ...
  async input(address: string, value: string) {
      const sendResult = await writeContract(this.viteChannelAddress,
          this.viteChannelAbi,
          'input',
          [ address, value ],
          this.tokenId,
  ...
  async inputIndex() {
      return readContract(this.viteProvider,
...
async function writeContract(to: string,
    abi: Array<{ name: string; type: string }>,
    methodName: string,
    params: any[],
    tokenId: string,
    amount: string) {
    const methodAbi = abi.find(x => x.name === methodName && x.type === 'function');
    if (!methodAbi) {
...
async function readContract(provider: any,
    to: string,
    abi: Array<{ name: string; type: string }>,
    code: any,
    methodName: string,
